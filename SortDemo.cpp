...
BEGIN_MESSAGE_MAP(CSortDemoApp, CWinApp)
	ON_COMMAND(ID_APP_ABOUT, &CSortDemoApp::OnAppAbout)
	// 依據文件命令的標準檔案
...
CSortDemoApp::CSortDemoApp() noexcept
{
	...
	SetAppID(_T("SortDemo.AppID.NoVersion"));

	// TODO:  在此加入建構程式碼，
	// 將所有重要的初始設定加入 InitInstance 中
}

// 唯一一個 CSortDemoApp 物件
...
BOOL CSortDemoApp::InitInstance()
{
	...
	pDocTemplate = new CSingleDocTemplate(
		IDR_MAINFRAME,
		RUNTIME_CLASS(CSortDemoDoc),
		RUNTIME_CLASS(CMainFrame),       // 主 SDI 框架視窗
		RUNTIME_CLASS(CSortDemoView));
	if (!pDocTemplate)
		return FALSE;
